    ...
    def transform(self, X):
        ...
        if 'total_charges' in X.columns and self.q1 is not None:
            def segmentar_total_charges(x):
                if x <= self.q1: return 'Bajo'
                if x <= self.q2: return 'Medio-Bajo'
                if x <= self.q3: return 'Medio-Alto'
                return 'Alto/VIP'
        ...
        if 'late_payments' in X.columns and 'tickets_grouped' in X.columns:
            X['cliente_problematico'] = ((X['late_payments'] + X['tickets_grouped']) > 0).astype(int)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"[OK] Feature Engineering completado. Columnas finales: {list(df_final.columns)}")
